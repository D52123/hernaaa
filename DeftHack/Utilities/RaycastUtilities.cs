...
public static class RaycastUtilities
{
    ...
    public static bool NoShootthroughthewalls(Transform transform)
    {
        ...
        if (localPlayer?.look?.aim == null) return false;
        
        Vector3 direction = AimbotCoroutines.GetAimPosition(transform, "Skull") - localPlayer.look.aim.position;
        return Physics.Raycast(new Ray(localPlayer.look.aim.position, direction), out RaycastHit raycastHit, direction.magnitude, RayMasks.DAMAGE_CLIENT, QueryTriggerInteraction.UseGlobal) && raycastHit.transform.IsChildOf(transform);
    }

 
    public static RaycastInfo GenerateOriginalRaycast(Ray ray, float range, int mask)
    {
        Physics.Raycast(ray, out RaycastHit hit, range, mask, QueryTriggerInteraction.UseGlobal);
    ...
    public static bool GenerateRaycast(out RaycastInfo info)
    {
        ...
        if (OptimizationVariables.MainPlayer == null || OptimizationVariables.MainPlayer.equipment == null || 
            OptimizationVariables.MainPlayer.look == null || OptimizationVariables.MainPlayer.look.aim == null)
        {
            info = default(RaycastInfo);
            return false;
        }
    ...
    public static bool GetTargetObject(GameObject[] Objects, out GameObject Object, out Vector3 Point, double Range)
    {
        ...
        Object = null;
        Point = Vector3.zero;
            if (OptimizationVariables.MainPlayer?.look?.aim == null)
            {
                return false;
            }
            Vector3 position = OptimizationVariables.MainPlayer.look.aim.position;
        ...
        foreach (GameObject gameObject in Objects)
        {
            ...
            if (component != null && (component.life.isDead || FriendUtilities.IsFriendly(component) || (RaycastOptions.NoShootthroughthewalls && !RaycastUtilities.NoShootthroughthewalls(gameObject.transform))))
                continue;
