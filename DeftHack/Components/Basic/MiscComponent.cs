...
public class MiscComponent : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        {
            // OptimizationVariables.MainCam = Camera.main; // MainCam is read-only, cannot assign
        }
        ...
        if (!flag2)
        {
            ...
            if (!flag3)
            {
                if (MiscOptions.hang)
                {
                    Player.instance.movement.pluginGravityMultiplier = 0f;
                }
                else
                {
                    Player.instance.movement.pluginGravityMultiplier = 1f;
                }
