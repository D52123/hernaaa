...
public static class MiscTab
{
    ...
    {
        bool fuck = bool_0;
        InteractableVehicle vehicle = UnturnedAPI.LocalPlayer.movement.getVehicle();
        if (vehicle != null)
        {
    ...
    { 
        Prefab.ScrollView(new Rect(0f, 0f, 466f, 436f), "Прочее", ref StatsTab.ScrollPos, delegate ()
        {
            ...
            if (MiscOptions.EnVehicle == true)
            {
                ...
                {
                    InteractableVehicle vehicle1 = UnturnedAPI.LocalPlayer.movement.getVehicle();
                    if (vehicle1 != null)
                    {
            ...
            if (flag2)
            {
                // bool flag3 = !OptimizationVariables.MainPlayer.look.isOrbiting; // isOrbiting property doesn't exist
                // if (flag3)
                // {
                ...
                {
                    if (Prefab.Button("Вернуть камеру", 150f, 25f, new GUILayoutOption[0]))
                    {
                        OptimizationVariables.MainPlayer.look.orbitPosition = Vector3.zero;
                    }
                }
        ...
        Prefab.MenuArea(new Rect(17f, 291f, 215f, 135f), "СПАМЕР", delegate
        {
            ...
            GUILayout.Space(5f);
            Prefab.Toggle("Дерганье головой", ref MiscOptions.SpammerEnabled, 17);
            GUILayout.Space(5f);
            GUILayout.Label("Задержка: " + MiscOptions.SpammerDelay.ToString("0.0") + "s", Prefab._TextStyle, new GUILayoutOption[0]);
            MiscOptions.SpammerDelay = (int)GUILayout.HorizontalSlider((float)MiscOptions.SpammerDelay, 0.1f, 5f, System.Array.Empty<GUILayoutOption>());
        });
