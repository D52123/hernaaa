        ...
        Prefab.MenuArea(new Rect(0f, 0f, 466f, 436f), "Опции", delegate
        {
            ...
            if (Prefab.Button("Применить", 200f, 25f, new GUILayoutOption[0]))
            {
                if (Provider.isConnected)
                {
                    ...
                    SteamConnectionInfo steamConnectionInfo = new SteamConnectionInfo(Provider.CurrentServerAdvertisement.ip, 0, "");
                    Provider.disconnect();
                    Thread.Sleep(50);
                    MenuPlayConnectUI.connect(steamConnectionInfo, true, MenuPlayServerInfoUI.EServerInfoOpenContext.CONNECT);
                }
