...
public static class StatsTab
{
    ...
    public static void Tab()
    {
        ...
        Prefab.MenuArea(new Rect(260f, 260f, 196f, 174f), "Счётчик убийств", delegate
        {
            ...
            GUILayout.Space(5f);
            if (Prefab.Button("Найти оружие", 167f, 25f) && UnturnedAPI.LocalPlayer)
            {
                Asset asset = UnturnedAPI.GetPlayerEquipmentAsset(UnturnedAPI.LocalPlayer);
                ItemAsset itemAsset = asset as ItemAsset;
                if (itemAsset != null)
                {
                    UnturnedAPI.LocalPlayer.equipment.getUseableStatTrackerValue(out EStatTrackerType estatTrackerType, out int num);

                    name = itemAsset.itemName.ToString();
                    id = itemAsset.id.ToString();
                    count = num.ToString();
                    labels = "Оружие:";
                }
