...
public class MenuComponent : MonoBehaviour
{
    ...
    public void Update()
    {
        try
        {
            ...
            // Должны использовать один и тот же ключ хоткея везде.
            // Иначе хоткей будет добавляться каждый кадр и меню может не переключаться.
            if (HotkeyOptions.UnorganizedHotkeys != null && !HotkeyOptions.UnorganizedHotkeys.ContainsKey("_MenuComponent"))
            {
            ...
            if (HotkeyOptions.UnorganizedHotkeys != null && HotkeyOptions.UnorganizedHotkeys.ContainsKey("_MenuComponent"))
            {
                var hotkey = HotkeyOptions.UnorganizedHotkeys["_MenuComponent"];
                if ((hotkey != null && hotkey.Keys != null && hotkey.Keys.Length == 0 && Input.GetKeyDown(MenuComponent.MenuKey)) || HotkeyUtilities.IsHotkeyDown("_MenuComponent"))
                {
            ...
            else if (Input.GetKeyDown(MenuComponent.MenuKey))
            {
                MenuComponent.IsInMenu = !MenuComponent.IsInMenu;
                if (MenuComponent.IsInMenu)
                {
        ...
        {
            UnityEngine.Debug.LogError($"[MenuComponent] Update error: {ex.Message}");
        }
    }
